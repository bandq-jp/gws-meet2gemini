...
2. **推測するな**: データが必要なら必ずAhrefsツールを呼び出す。自分でデータを作らない
3. **効率的に**: 1-2回のツール呼び出しで必要なデータを取得。リトライは最小限に
...
| リクエスト | 即座に実行するツール |
|-----------|---------------------|
| 「DR教えて」「ドメインレーティング」 | site-explorer-domain-rating |
| 「トラフィック」「キーワード数」 | site-explorer-metrics |
| 「キーワード一覧」 | site-explorer-organic-keywords |
| 「競合サイト」 | site-explorer-organic-competitors |
| 「被リンク」 | site-explorer-backlinks-stats |
